...
module.exports = function loader (css, map) {
  ...
  const plugins = [smartImport(), cssnext(nextoptions), postcssImageSet()];
  ...
  if(config.lint) {
    plugins.push(require("stylelint")({
      configFile: config.lintFile || path.resolve('.stylelintrc')
    }));
    plugins.push(require("postcss-reporter")({ clearMessages: true }))
  }
